...
def load_mlp(path, input_dim=990, num_classes=2):
...
def load_cnn(path, input_channels=1, num_classes=2):
    state = torch.load(path, map_location=device)
...
step2_mlps = [
    load_mlp(f"classifier/ensemble/step2/step2_{i}.pt") for i in range(3)
]
step2_cnn = load_cnn("classifier/ensemble/step2/step2_3.pt")
step2_xgb = joblib.load("classifier/ensemble/step2/step2_4.pkl")

step2a_mlps = [
    load_mlp(f"classifier/ensemble/step2A/step2A_{i}.pt") for i in range(2)
]
step2a_cnn = load_cnn("classifier/ensemble/step2A/step2A_2.pt")
step2a_xgb = joblib.load("classifier/ensemble/step2A/step2A_3.pkl")

step2b_mlps = [
    load_mlp(f"classifier/ensemble/step2B/step2B_{i}.pt") for i in range(2)
]
...
def predict_tree(x_flat: np.ndarray) -> int:
    ...
    x_seq_tensor = x_tensor.view(1, 10, 99)
    x_seq_cnn = x_seq_tensor.unsqueeze(1)  # (1, 1, 10, 99)
    ...
    cnn_logits = step2_cnn(x_seq_cnn).detach().cpu().numpy()
    xgb_logits = step2_xgb.predict_proba(x_flat)
    step2_logits = np.mean([mlp_logits, cnn_logits, xgb_logits], axis=0)
    pred_step2 = np.argmax(step2_logits)

    if pred_step2 == 1:
        # Step 2A
        mlp_logits = soft_vote(step2a_mlps, x_tensor)
        cnn_logits = step2a_cnn(x_seq_cnn).detach().cpu().numpy()
